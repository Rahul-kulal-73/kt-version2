...
import { Search, Edit2, MoreVertical, Trash2, ArrowUpRight, Eye } from 'lucide-react';
...
export function UserTable({ users, onEdit, onSearch }: UserTableProps) {
  ...
  const [feedbackUser, setFeedbackUser] = useState<IUser | null>(null);
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-slate-400 hover:text-[#1b7c7c] hover:bg-[#1b7c7c]/10"
                          onClick={() => setFeedbackUser(user)}
                          title="View Feedback"
                        >
